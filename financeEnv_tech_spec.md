...
All submissions must pass the following automated verification steps:

* **HF Space Deploys**: Automated ping to the Space URL. Must return `200` and respond to `reset()`.
* **OpenEnv Spec Compliance**: Validation of `openenv.yaml`, typed models, and `/step()`, `/reset()`, `/state()` endpoints.
* **Dockerfile Builds**: Automated `docker build` on the submitted repository.
* **Baseline Reproduces**: Execution of the submitted `inference.py` script. Must complete without error and produce scores.
* **3+ Tasks with Graders**: Enumerate tasks, run each grader, and verify scores/rewards are in the `0.0–1.0` range.

---

## 2. Mandatory Configuration
The following variables must be defined in your environment configuration:
...
* **File Name**: The inference script must be named `inference.py` and placed in the root directory.
* **LLM Client**: Participants **must** use the OpenAI Client for all LLM calls using the variables above.
* **Structured Logs**: Participants must emit stdout logs strictly following the `[START]`, `[STEP]`, and `[END]` format. 
    * *Note: Any deviation in field names, ordering, or formatting will result in incorrect evaluation scoring.*

---

## 3. Infrastructure & Runtime Limits
* **Runtime**: Inference script execution must be under **20 minutes**.
* **Hardware Profile**: Environment and inference must run on a machine with **2 vCPUs** and **8GB RAM**.

## 4. Validator
**Action Required**: Run the pre-submission validation script before final submission.
